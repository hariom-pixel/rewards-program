...
describe('calculateTotalRewards', () => {
  it('should return correct rewards for multiple transactions of a single customer', () => {
    const transactions = [
      { customerId: 'C1', customerName: 'Alice', price: 55 },
    ]
    const expectedRewards = [{ customerName: 'Alice', rewardPoints: 5 }] // 10 (60-50)*1 + 135 [(150-100)*2 + 50*1]
    expect(calculateTotalRewards(transactions)).toEqual(expectedRewards)
  })

  it('should return correct rewards for multiple customers', () => {
    const transactions = [
      { customerId: 'C1', customerName: 'Alice', price: 60 },
      { customerId: 'C2', customerName: 'Bob', price: 150 },
    ]
    const expectedRewards = [
      { customerName: 'Alice', rewardPoints: 10 },
      { customerName: 'Bob', rewardPoints: 150 },
    ]
    expect(calculateTotalRewards(transactions)).toEqual(expectedRewards)
  })
  ...
  it('should return 0 rewards for customers with no valid transactions', () => {
    const transactions = [
      { customerId: 'C1', customerName: 'Alice', price: 30 },
      { customerId: 'C2', customerName: 'Bob', price: 40 },
    ]
    const expectedRewards = [
      { customerName: 'Alice', rewardPoints: 0 },
      { customerName: 'Bob', rewardPoints: 0 },
    ]
    expect(calculateTotalRewards(transactions)).toEqual(expectedRewards)
  })
})
